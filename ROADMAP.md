...
| **v5** | **Fresh-assurance lane** | session VINKs (`live_presence`/`rightful_holder`) with TTL; fresh-at-use, never persisted | ✅ **live** (`fresh_v5.json`) |
| v6 | Offer-first ceremony | the stages OFFER→…→MATERIALIZE; the T-1→T0 "never auto-bind" rule | 🔜 **next** |
